...
public class DiagramDrawerConstructedDiagramForceModel extends DiagramDrawer{
	

	///Class Default Values 
	private static final int DEFAULT_ACCELERATORKEY = KeyEvent.VK_S; 
	
	protected static final int DEFAULT_I = 100;  					
	
	protected static final double DEFAULT_A = 0.35;      		//0.15;//0.28//0.04; //0.2 //0.01
	...
	protected static final double DEFAULT_ZM_IZ_A = 0.01;		//.02;   //.018; //0.015//0.01; //0.02;		//0.04; .01; //0.2;
	protected static final double DEFAULT_ZM_NIZ_A = 0.02;  	//.02;//0.025; 0.02; //0.15;		//0.04; .01; //0.2;
	
	protected static final double DEFAULT_R = 1200;        	    //10000; //90000 //1000
	...
	protected static final double DEFAULT_ZM_IZ_R = 50;   		//30;   //60;//100;		//10; //000; //50; //100000;	
	protected static final double DEFAULT_ZM_NIZ_R = 20;   		//20;   //30;//100;		//4000;//10; //000; //50; //100000;

	protected static final double DEFAULT_M = .9;   	      	//1.5; //1

	protected static final double DEFAULT_MAX_ABS_FORCE = 80;
	
	protected static final double DEFAULT_AT = 100;       
	protected static final double DEFAULT_RT = 20;      
	protected static final boolean at_on = false;        		//true; //set at to 100 -> for 4 sets; //false;
	protected static final boolean rt_on = false;        		//set rt to 20 - for 4 sets
	...
	protected double maxAbsForce = DEFAULT_MAX_ABS_FORCE; 
	...
	public DiagramDrawerConstructedDiagramForceModel(){ //(DiagramPanel inDiagramPanel) {
		super(DEFAULT_ACCELERATORKEY, "Force Directed", DEFAULT_ACCELERATORKEY);
		//setDiagramPanel(inDiagramPanel);
		setAll();
	}
	
	public DiagramDrawerConstructedDiagramForceModel(int inAcceleratorKey){ //, DiagramPanel inDiagramPanel) {
		super(inAcceleratorKey, "Force Directed", inAcceleratorKey);
		//setDiagramPanel(inDiagramPanel);
		setAll();
	}

	public DiagramDrawerConstructedDiagramForceModel (int inAcceleratorKey, int inI, double inA, double inCM_OC_A, double inCM_NOC_A, double inZM_IZ_A, double inZM_NIZ_A, double inR, double inCM_OC_R, double inCM_NOC_R, double inZM_IZ_R, double inZM_NIZ_R, double inM){ //, DiagramPanel inDiagramPanel){
		super (inAcceleratorKey, "Force Directed", inAcceleratorKey);
		//setDiagramPanel(inDiagramPanel);
	...
	public void layout() {
		...
		System.out.println("--- Force Model is Being Applied ---");
					...
					*/
					...
					for (ArrayList<Polygon> zpolys : zspolys){
						...
						for (Polygon zpoly : zpolys){
							...
							boolean pntShallBeInZone = zpoly.contains(newPntPost);
							
							Point zoneInternalMiddlePnt = HybridGraph.findMiddlePointInsidePolygon(zpoly);
							Point2D.Double zoneInternalMiddlePnt_Dbl = new Point2D.Double(zoneInternalMiddlePnt.x, zoneInternalMiddlePnt.y);
							
							if (pntShallBeInZone && pntOrigInZone){
								xyForceOnPnt = addxyZoneMiddleAttraction_PointInZone(xyForceOnPnt, newPntPost, zoneInternalMiddlePnt_Dbl);
								xyForceOnPnt = addxyZoneMiddleRepulsion_PointNotInZone(xyForceOnPnt, zoneInternalMiddlePnt_Dbl, newPntPost);
							} else if (pntShallBeInZone && !pntOrigInZone){
								xyForceOnPnt = addxyZoneMiddleRepulsion_PointInZone(xyForceOnPnt, pnt, getZoneMiddleRepulsion_PointInZone_Pnt2(p, cci, zoneInternalMiddlePnt_Dbl, origZonesForPnts, zonesNames, zspolys));
							} else if (!pntShallBeInZone && pntOrigInZone){
								xyForceOnPnt = addxyZoneMiddleAttraction_PointNotInZone(xyForceOnPnt, newPntPost, zoneInternalMiddlePnt_Dbl);
							} else if (!pntShallBeInZone && !pntOrigInZone){
								xyForceOnPnt = addxyZoneMiddleRepulsion_PointNotInZone(xyForceOnPnt, newPntPost, zoneInternalMiddlePnt_Dbl);
							}
						}
						z++;
					}
		...
		System.out.println("--- Force Model Applied ---");
	}
	...
	// Identify the second point which should be used to calculate the zone middle repulsion force when a point is unintentionally is in a zone 
	protected Point2D.Double getZoneMiddleRepulsion_PointInZone_Pnt2(int pntIndexOnContour, int pntContourIndex, Point2D.Double zoneMiddle, HashMap<String, ArrayList<String>>origZonesForPnts, ArrayList<String> zonesnames, ArrayList<ArrayList<Polygon>> zspolys){
		
		Polygon zoneContainingPoint = findFirstZoneContainingPoint(pntIndexOnContour, pntContourIndex, origZonesForPnts, zonesnames, zspolys);
	    
		if (zoneContainingPoint == null){
	    	return zoneMiddle;
	    } else {
	    	Point middleOfZoneContainingPoint = HybridGraph.findMiddlePointInsidePolygon(zoneContainingPoint);
	    	return (new Point2D.Double(new Double(middleOfZoneContainingPoint.x), new Double(middleOfZoneContainingPoint.y)));
	    }
	}
	...
	protected Point2D.Double addxyForce(ForceType ft, Point2D.Double xyForce, Point2D.Double pnt1, Point2D.Double pnt2){
		...
		Point2D.Double new_xyForce = new Point2D.Double();
	
		new_xyForce.x = xyForceDirection.x * (xyDistance.x/(xyDistance.x+xyDistance.y)) * totalForce;
		new_xyForce.y = xyForceDirection.y * (xyDistance.y/(xyDistance.x+xyDistance.y)) * totalForce;
	
		
		// Check for Infinite or NaN
		
		if (Double.isInfinite(new_xyForce.x)){
			new_xyForce.x = xyForceDirection.x * maxAbsForce;
		} else if (Double.isNaN(new_xyForce.x)){
			new_xyForce.x = 0;
		}
		
		if (Double.isInfinite(new_xyForce.y)){
			new_xyForce.y = xyForceDirection.y * maxAbsForce;
		} else if (Double.isNaN(new_xyForce.y)){
			new_xyForce.y = 0;
		}
		
		
		// Update the force for point 
		
		xyForce.x += new_xyForce.x;
		xyForce.y += new_xyForce.y;
	...
	protected Point2D.Double getNewPntPos (Point2D.Double xyForce, Point2D.Double pnt){
		...
		// Ensure that the force to be applied is between limits 
		
		if (xyForce.x > maxAbsForce){
			xyForce.x = maxAbsForce;
		} else if (xyForce.x < (-maxAbsForce)){
			xyForce.x = - maxAbsForce;
		}
		
		if (xyForce.y > maxAbsForce){
			xyForce.y = maxAbsForce;
		} else if (xyForce.y < (-maxAbsForce)){
			xyForce.y = - maxAbsForce;
		}
			
		// Get the new position for the point 
		
		Point2D.Double newPos = new Point2D.Double();
		
		newPos.x = Math.rint(m * xyForce.x) + pnt.x; 
		newPos.y = Math.rint(m * xyForce.y) + pnt.y;
		
		return (newPos);
	}
	

	// Find and return the first zone polygons which contains the given point and which is not part of the null (outer) zone
	protected Polygon findFirstZoneContainingPoint (int pntIndexOnContour, int pntContourIndex, HashMap<String, ArrayList<String>>origZonesForPnts, ArrayList<String> zonesNames, ArrayList<ArrayList<Polygon>> zspolys){

		String pntID = Integer.toString(pntContourIndex) + Integer.toString(pntIndexOnContour);
		
		ArrayList<String> pntOrigInZone = ((ArrayList<String>)origZonesForPnts.get(pntID));
		int i = 0;
		int zoneIndex = 0;
		String reqZoneName = "";
		do{
			//Random random = new Random();
			//int randomOrigZoneIndex = random.nextInt(pntOrigInZone.size());
			reqZoneName = pntOrigInZone.get(i);
			zoneIndex = zonesNames.indexOf(reqZoneName);
			i++;
		} while ((reqZoneName == "") && (i < pntOrigInZone.size()));
		
		try{
			if ((i <= pntOrigInZone.size()) && (reqZoneName != "")){
				return (zspolys.get(zoneIndex).get(0));
			} else {
				return null;
			}
