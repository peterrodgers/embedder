...
public class PolygonIntersect extends Object
{
    ...
    */
	 public static int intersectionArea(Polygon polygon1, Polygon polygon2)
	   {
		PolygonIntersect polygonIntersect = new PolygonIntersect();
		Point2D[] a = new Point2D[polygon1.npoints];
		Point2D[] b = new Point2D[polygon2.npoints];
		for(int i = 0 ; i < polygon1.npoints; i++){
			a[i] = new Point2D.Double(polygon1.xpoints[i], polygon1.ypoints[i]);			
		}
		for(int j = 0 ; j < polygon2.npoints; j++){
			b[j] = new Point2D.Double(polygon2.xpoints[j], polygon2.ypoints[j]);			
		}
		float temp = (float)polygonIntersect.inter(a, b);
   ...
   private void  fit(Point2D[] x, int cx, Vertex[] ix, int fudge, Box B)
   {
	int c = cx;
	while (c-- > 0) {
	    ix[c] = new Vertex();
   ...
   private double inter(Point2D[] a, Point2D[] b)
   {
	int na = a.length;
	int nb = b.length;
