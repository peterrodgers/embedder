...
public class AddDualPiercingDialog extends JDialog implements ActionListener {
	...
	String dual1Text;
	JTextField dual1Field;
	String dual2Text;
	JTextField dual2Field;
...
/** Edge list must have at least one element */
	public AddDualPiercingDialog(PiercingDiagramWindow pdw, Frame containerFrame) {
	...
	protected void addWidgets(JPanel widgetPanel, GridBagLayout gridbag) {
		...
		dual1Field = new JTextField(32);
		dual1Field.setText(dual1Text);
		dual1Field.setCaretPosition(0);
		
		JLabel dual1 = new JLabel("Pierced to curve 1: ", SwingConstants.LEFT);
	
		
		dual2Field = new JTextField(32);
		dual2Field.setText(dual2Text);
		dual2Field.setCaretPosition(0);
		
		JLabel dual2 = new JLabel("Pierced to curve 2: ", SwingConstants.LEFT);
		...
		c.gridy = yLevel;
		c.anchor = GridBagConstraints.EAST;
		gridbag.setConstraints(dual1,c);
		widgetPanel.add(dual1);

		c.gridx = 1;
		c.gridy = yLevel;
		c.anchor = GridBagConstraints.WEST;
		gridbag.setConstraints(dual1Field,c);
		widgetPanel.add(dual1Field);
		...
		c.gridy = yLevel;
		c.anchor = GridBagConstraints.EAST;
		gridbag.setConstraints(dual2,c);
		widgetPanel.add(dual2);

		c.gridx = 1;
		c.gridy = yLevel;
		c.anchor = GridBagConstraints.WEST;
		gridbag.setConstraints(dual2Field,c);
		widgetPanel.add(dual2Field);
	...
	public void okButton(ActionEvent event) {
		...
		String dual1 = dual1Field.getText();
		String dual2 = dual2Field.getText();
		String outer = outerCurveField.getText();
		if(piercingDiagramWindow.getPiercingDiagram().getCurve(dual1)==null
				||piercingDiagramWindow.getPiercingDiagram().getCurve(dual2)==null){
			...
			if(curveLabel!= null){
				if(outer==null)
					outer="";
				DualPiercingCurve dpc = new DualPiercingCurve(curveLabel,dual1,dual2,outer);
				dpc.setIsDualPiercing(true);
				PiercingDiagram pd = piercingDiagramWindow.getPiercingDiagram();
								pd.addNewDualPiercing(dpc);				
