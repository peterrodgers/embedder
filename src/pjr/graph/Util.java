...
public class Util {
	...
	public static double getRelativeAngle(Point p1, Point p2, Point p3, Point relativePoint) {
		...
		double angle = pjr.graph.Util.angle(p1,p2,p3);
		
		double relativeP1Angle = pjr.graph.Util.angle(p1,p2,relativePoint);
		double relativeP3Angle = pjr.graph.Util.angle(p3,p2,relativePoint);
	...
	public static Point2D.Double getExtendedPoint(Point2D p1, Point2D p2, double dis, Face face, String label){
		...
		if(p1.getY()>=p2.getY())
		 ...
		 if(face.generatePolygon().contains(x,y)){
			 if(face.getNodeList().size() == 4){
				 Point2D p = getExtendedPoint(face.getCentre(label),p2,dis);
				 x  = p.getX();
				 y =  p.getY();
			 }
			 else{
				 Point2D p3 = face.getCentre(label);
				 Point2D p = getExtendedPoint(p3,p2,dis);
				 x  = p.getX();
				 y =  p.getY();
			}
		}
	...
	/** Return true if the polygon is concave, false if it is convex */
	public static boolean isConcave(Polygon p)
	{
		...
		// remember to skip the last index, as it has the same coords the 1st index has 
		Coord first = coords[0];
		Coord last = coords[len-1];
		boolean closed = (first.lon == last.lon && first.lat == last.lat);
		
		assert (closed && len >= 4) || (!closed && len >= 3); // must have 2 or more vertices
		
		if ((!closed && len == 3) || (closed && len == 4))
			return false; // triangle is always convex
		...
		int lastIdx = closed? len-2 : len-1;
		for (int i=3; i<=lastIdx; i++)
		{
			double xp = crossProduct(coords[i-2], coords[i-1], coords[i]);
			if (xp != 0 && !sameSign(xp1, xp))
				return true;
		}
		
		int im2 = closed? len-3 : len-2;
		int im1 = closed? len-2 : len-1;
		
		double xpm2 = crossProduct(coords[im2], coords[im1], coords[0]);
		double xpm1 = crossProduct(coords[im1], coords[0], coords[1]);

		if (xpm1 != 0 && !sameSign(xp1, xpm1))
			return true;
		if (xpm2 != 0 && !sameSign(xp1, xpm2))
			return true;
	...
	public static double crossProduct(Coord prev, Coord center, Coord next)
	{
		double x  = center.lon; // center
		double y  = center.lat;
		double xn = next.lon; // next
		double yn = next.lat;
		double xp = prev.lon; // prev
		double yp = prev.lat;
